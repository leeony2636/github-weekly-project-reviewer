...
class TraceStore:
    ...
    def _initialize(self) -> None:
        ...
        self._ensure_column(
            "review_runs",
            "run_kind",
            "TEXT NOT NULL DEFAULT 'pull_request'",
        )
        self._ensure_column(
            "review_runs",
            "subject_id",
    ...
    def start_run(
        ...
        *,
        ...
        if run_kind not in {
            ...
            raise ValueError(
                "run_kind는 pull_request 또는 "
                "weekly여야 합니다."
            )

        if run_kind == "pull_request":
            if pull_request_number < 1:
                raise ValueError(
                    "pull_request_number는 "
                    "1 이상이어야 합니다."
                )
