...
class TraceStoreTests(unittest.TestCase):
    ...
    def test_starts_pull_request_run(self) -> None:
    ...
    def test_starts_weekly_run(self) -> None:
        run_id = self.store.start_run(
            repository="owner/repository",
            pull_request_number=0,
            run_kind="weekly",
            subject_id="2026-09-01_2026-09-08",
            diff_sha256="a" * 64,
        )
    ...
    def test_saves_consensus_findings(self) -> None:
        ...
        findings = (
            self.store.get_consensus_findings(
                run_id
            )
        )
    ...
    def test_records_security_block_status(
        self,
    ) -> None:
        run_id = self.store.start_run(
            repository="owner/repository",
